...
open class GooglePlacesAutocomplete: UINavigationController {
  ...
  public convenience init(apiKey: String, placeType: PlaceType = .all) {
    ...
    gpaViewController.navigationController?.navigationBar.barTintColor = UIColor.init(colorLiteralRed: 45.0/255.0, green: 190.0/255.0, blue: 221.0/255.0, alpha: 1.0)
    gpaViewController.navigationController?.navigationBar.tintColor = UIColor.white
...
open class GooglePlacesAutocompleteContainer: UIViewController {
  ...
  override open func viewDidLoad() {
    ...
    let customView = UIView(frame: CGRect(x: 0, y: 0, width: screenSize.width, height: 45))
    let lblText = UILabel(frame: CGRect(x: 0 , y: 0, width: screenSize.width, height: 45))
    lblText.text = "Please make sure the place is in Qatar"
    lblText.textColor = UIColor.white
    lblText.font = UIFont.systemFont(ofSize: 16)
    lblText.textAlignment = NSTextAlignment.center


    customView.addSubview(lblText)
    ...
    searchBar.placeholder = "Please make sure the place is in Qatar"
  ...
  func keyboardWasShown(_ notification: Notification) {
    if isViewLoaded && view.window != nil {
      ...
      let keyboardSize: CGSize = (info[UIKeyboardFrameBeginUserInfoKey] as! NSValue).cgRectValue.size//((info[UIKeyboardFrameBeginUserInfoKey] as AnyObject).cgRectValue.size)
...
class GooglePlacesRequestHelpers {
  ...
  */
  ...
  fileprivate class func doRequest(_ url: String, params: [String: String], completion: @escaping (NSDictionary?,NSError?) -> ()) {
    ...
    var strQuery:String = query(params as [String : AnyObject])
    strQuery = strQuery.replacingOccurrences(of: "Optional%28", with: "")
    strQuery = strQuery.replacingOccurrences(of: "%29", with: "")


    let request = URLRequest(url: URL(string: "\(url)?\(strQuery)")!)
